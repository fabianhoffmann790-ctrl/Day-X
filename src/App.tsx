    ...
    return (
      <main className="menu-screen">
        <section className="menu-panel">
          ...
          <h1>Day-X</h1>
          <p>Verlassene Siedlungen, knappe Ressourcen, gefährliche Infizierte und langsames taktisches Überleben. Alles ist ein eigenständiger Platzhalter-Prototyp ohne fremde Markenassets.</p>
          <div className="menu-grid"><span>Körperzustand</span><span>Kleidung</span><span>Item-Zustand</span><span>Crafting</span><span>Lagerfeuer</span><span>Gewicht</span></div>
  ...
  return (
    <main className="game-screen">
      ...
      <section className="hud">
        <div className="meta-row"><span>Zeit: {hud.timeText}</span><span>Wetter: {weatherLabels[hud.weather]}</span><span>Geräusch: {hud.noiseLevel}</span><span>Feuerwärme: {hud.nearbyFireWarmth}%</span></div>
        ...
        <div className="quick-row"><span>Inventar: {hud.usedSlots}/{hud.capacity}</span><span>Gewicht: {hud.totalWeight} kg</span><span>Schutz: {Math.round(hud.armor * 100)}%</span><span>Wärme: {hud.warmth}</span><span>Regen: {hud.rainProtection}</span></div>
        <div className="quick-row"><span>Rucksack: {hud.currentBackpack}</span><span>Weste/Rüstung: {hud.currentArmor}</span></div>
      ...
      {hud.inventoryOpen && (
        <aside className="inventory">
          ...
          <div className="equipment-grid">{Object.entries(hud.clothing).map(([slot, value]) => <div key={slot}><strong>{slotLabels[slot as keyof typeof slotLabels]}</strong><span>{value}</span></div>)}</div>
          {hud.inventory.length === 0 ? <p>Leer. Suche Gebäude ab.</p> : null}
          <ul>{hud.inventory.map((entry) => <li key={entry.id} className={entry.equipped ? 'equipped' : ''}><div><strong>{entry.name}</strong><small>{entry.type} · {entry.conditionLabel}{entry.freshness ? ` · ${entry.freshness}` : ''}</small><small>Slots: {entry.size} · Gewicht: {entry.totalWeight} kg</small><small>{entry.canUse ? 'verwendbar' : ''}{entry.canUse && entry.canEquip ? ' · ' : ''}{entry.canEquip ? 'ausrüstbar' : ''}</small></div><span>x{entry.count}{entry.equipped ? ' ✓' : ''}</span></li>)}</ul>
        </aside>
      )}
      {hud.craftingOpen && <aside className="crafting-panel"><h2>Crafting</h2><p className="inventory-help">Drücke 4-9 oder 0 für das entsprechende Rezept. Aktionen brechen bei Bewegung oder Treffer ab.</p><ol start={4}>{hud.craftingRecipes.map((recipe) => <li key={recipe.id} className={recipe.available ? 'available' : 'missing'}><strong>{recipe.name}</strong><small>{recipe.timeSeconds}s {recipe.available ? 'bereit' : `fehlt: ${recipe.missing.join(', ')}`}</small></li>)}</ol></aside>}
