...
export type EquipmentSlot = 'primary' | 'secondary' | 'melee' | ClothingSlot;
...
export interface InventoryEntry { id: string; name: string; type: ItemType; count: number; size: number; weight: number; totalWeight: number; condition: ItemCondition; conditionLabel: string; freshness?: FoodFreshness; equipped: boolean; canUse: boolean; canEquip: boolean; description: string; }
export interface HotbarSlot { slot: number; itemId: string | null; label: string; }
export interface InventorySaveData { items: Array<[string, number]>; equippedWeaponId: string | null; equippedArmorId: string | null; equippedBackpackId: string | null; itemDurability?: Record<string, number>; itemFreshness?: Record<string, FoodFreshness>; clothingSlots?: Partial<Record<ClothingSlot, string | null>>; hotbar?: Array<[number, string | null]>; }
...
export interface DebugHudState { godMode: boolean; debugPanelOpen: boolean; playerPosition: string; zombieCount: number; lootSpotCount: number; droppedItemCount: number; }
...
export interface HudState {
  ...
  zombiesAlerted: number; interactionPrompt: string; message: string; warning: string; inventoryOpen: boolean; craftingOpen: boolean; craftingRecipes: CraftingRecipeView[]; inventory: InventoryEntry[];
...
export interface DroppedItemSaveData { id: string; itemId: string; count: number; position: { x: number; y: number; z: number }; }
