...
function isWorldAction(value: unknown): value is WorldActionLike { return typeof value === 'object' && value !== null && 'label' in value && 'duration' in value && 'complete' in value; }
...
export class GameEngine {
  ...
  dispose() { cancelAnimationFrame(this.animationId); this.canvas.removeEventListener('click', this.onCanvasClick); window.removeEventListener('resize', this.onResize); window.removeEventListener('keydown', this.onKeyDown); window.removeEventListener('keyup', this.onKeyUp); document.removeEventListener('mousemove', this.onMouseMove); document.removeEventListener('mousedown', this.onMouseDown); }
  ...
  private bindEvents() { this.canvas.addEventListener('click', this.onCanvasClick); window.addEventListener('resize', this.onResize); window.addEventListener('keydown', this.onKeyDown); window.addEventListener('keyup', this.onKeyUp); document.addEventListener('mousemove', this.onMouseMove); document.addEventListener('mousedown', this.onMouseDown); }
  private onCanvasClick = () => { this.sound.unlock(); this.canvas.requestPointerLock(); };
  ...
  private onKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.code === 'Tab') { event.preventDefault(); this.inventoryOpen = !this.inventoryOpen; this.sound.play('inventory', 0.15); this.raiseNoise(BALANCE.sound.inventoryNoiseRadius, 'Inventar raschelt.'); this.emitHud(); }
  ...
  private onMouseDown = (event: MouseEvent) => { if (event.button !== 0 || document.pointerLockElement !== this.canvas || this.activeAction) return; this.primaryAttack(); };
  ...
  private update(delta: number) {
    ...
    updatePlayerVitals(this.stats, delta, { weather: this.atmosphere.weather, isNight: this.atmosphere.isNight(), warmth: this.inventory.warmth, rainProtection: this.inventory.rainProtection, nearbyFireWarmth: this.nearbyFireWarmth, totalWeight: this.inventory.totalWeight });
    this.updateReload(delta);
    this.updateAction(delta);
    this.atmosphere.update(delta, this.camera.position);
    this.persistentWorld.update(delta, this.camera, this.camera.position, this.noiseRadius, this.atmosphere.isNight(), () => this.takeDamage(BALANCE.zombie.baseDamage, 0.2));
    this.updateInteractionPrompt();
    this.updateWarnings();
    this.damageFlash = Math.max(0, this.damageFlash - delta * 2.6);
    let alerted = 0;
    for (const zombie of this.zombies) { zombie.update(delta, { playerPosition: this.camera.position, noiseOrigin: this.camera.position, noiseRadius: this.noiseRadius, isNight: this.atmosphere.isNight(), weather: this.atmosphere.weather, onAttack: (damage, bleedChance) => this.takeDamage(damage, bleedChance) }); if (zombie.alive && zombie.alerted) alerted += 1; }
    this.noiseRadius = Math.max(0, this.noiseRadius - delta * 24);
    if (this.noiseRadius <= 0.2) this.noiseLabel = 'leise';
    this.autoSaveTimer += delta;
    if (this.autoSaveTimer > 20) { this.autoSaveTimer = 0; this.saveGame(false); }
  ...
  private updateMovement(delta: number) {
    ...
    if (direction.lengthSq() > 0) { direction.normalize().applyAxisAngle(new THREE.Vector3(0, 1, 0), this.yaw); this.camera.position.addScaledVector(direction, speed * delta); }
    if (sprinting) this.stats.stamina = Math.max(0, this.stats.stamina - BALANCE.vitals.sprintStaminaPerSecond * weight.drain * delta); else this.stats.stamina = Math.min(100, this.stats.stamina + (sneaking ? BALANCE.vitals.sneakStaminaRegenPerSecond : BALANCE.vitals.walkStaminaRegenPerSecond) * weight.regen * painRegen * illnessRegen * delta);
    if (this.pressed('Space') && this.grounded && this.stats.stamina > BALANCE.vitals.jumpStaminaCost && !this.activeAction) { this.verticalVelocity = 5; this.grounded = false; this.stats.stamina -= BALANCE.vitals.jumpStaminaCost; this.raiseNoise(BALANCE.sound.jumpNoiseRadius, 'Sprung landet laut.'); }
    this.verticalVelocity -= 12 * delta; this.camera.position.y += this.verticalVelocity * delta;
    if (this.camera.position.y <= 1.75) { this.camera.position.y = 1.75; this.verticalVelocity = 0; this.grounded = true; }
    this.camera.position.x = THREE.MathUtils.clamp(this.camera.position.x, -BALANCE.world.playerBounds, BALANCE.world.playerBounds); this.camera.position.z = THREE.MathUtils.clamp(this.camera.position.z, -BALANCE.world.playerBounds, BALANCE.world.playerBounds);
    if (moving) { const radius = sneaking ? BALANCE.sound.sneakNoiseRadius : sprinting ? BALANCE.sound.sprintNoiseRadius : BALANCE.sound.walkNoiseRadius; this.raiseNoise(radius); this.stepTimer -= delta; if (this.stepTimer <= 0) { this.sound.play(sprinting ? 'sprint' : 'footstep', sprinting ? 0.34 : 0.18); this.stepTimer = sprinting ? 0.32 : sneaking ? 0.82 : 0.55; if (sprinting) this.message = 'Du sprintest laut. Zombies in der Nähe könnten reagieren.'; } } else this.stepTimer = 0;
  }
  ...
  private interactLoot() { const target = this.hoveredLoot ?? this.findNearestReachableLoot(); if (!target?.spawnedLoot) { this.message = 'Nichts in Interaktionsreichweite. Schau direkt auf Loot, Tür oder Container und drücke E.'; return; } const item = ITEMS[target.spawnedLoot.itemId]; if (!item) return; const amount = target.spawnedLoot.count; if (!this.inventory.add(item.id, amount)) { this.message = `Inventar voll: Kein Platz für ${item.name}. Rucksack finden oder Storage-Kiste bauen.`; return; } target.taken = true; target.mesh.visible = false; target.respawnAt = performance.now() / 1000 + BALANCE.loot.respawnDelaySeconds; target.spawnedLoot = null; this.sound.play('pickup', 0.18); this.raiseNoise(BALANCE.sound.pickupNoiseRadius); this.message = `Aufgenommen: ${item.name}${amount > 1 ? ` x${amount}` : ''}.`; this.updateInteractionPrompt(); }
  ...
  private equipWeaponSlot(index: number) { this.inventory.cycleWeapon(index); const weapon = this.inventory.equippedWeapon(); this.message = weapon ? `Ausgerüstet: ${weapon.name}.` : 'Keine Waffe in diesem Slot.'; }
  ...
  private reloadEquippedWeapon() { const weapon = this.inventory.equippedWeapon(); const data = weapon?.weapon; if (!weapon || !data || data.kind !== 'ranged' || !data.ammoType || !data.magazineSize) { this.message = 'Diese Waffe muss nicht nachgeladen werden.'; return; } if (this.inventory.isRuined(weapon.id)) { this.message = `${weapon.name} ist ruiniert.`; return; } const loaded = this.magazineAmmo.get(weapon.id) ?? 0; const needed = data.magazineSize - loaded; const reserve = this.inventory.count(data.ammoType); if (needed <= 0) { this.message = 'Magazin ist bereits voll.'; return; } if (reserve <= 0) { this.message = `Keine passende Munition für ${weapon.name}.`; return; } this.startAction('Nachladen', data.reloadTime ?? 1, () => { const amount = Math.min(needed, this.inventory.count(data.ammoType ?? '')); if (!data.ammoType || amount <= 0) { this.message = 'Nachladen fehlgeschlagen: keine Munition mehr.'; return; } this.inventory.consumeAmmo(data.ammoType, amount); this.magazineAmmo.set(weapon.id, loaded + amount); this.sound.play('reload', 0.24); this.raiseNoise(BALANCE.sound.reloadNoiseRadius, 'Nachladen macht Geräusche.'); this.message = `${weapon.name} nachgeladen: ${loaded + amount}/${data.magazineSize}.`; }); }
  private takeDamage(rawDamage: number, bleedChance: number) { const result = applyDamage(this.stats, rawDamage, this.inventory.armor, bleedChance); this.activeAction = null; this.damageFlash = 1; this.sound.play('injury', 0.5); this.raiseNoise(BALANCE.sound.painNoiseRadius); this.message = result.startedBleeding ? `Zombieangriff! ${Math.round(result.damageTaken)} Schaden, du blutest.` : `Zombieangriff! ${Math.round(result.damageTaken)} Schaden. Abstand gewinnen.`; if (result.infectedByHit) this.message += ' Infektionsrisiko!'; }
  private startConsumeFood() { const item = this.inventory.entries().find((entry) => entry.type === 'food' && entry.canUse); if (!item) { this.message = 'Keine Nahrung im Inventar.'; return; } this.startAction(`Essen: ${item.name}`, 2.6, () => { const consumed = this.inventory.useBestFood(); if (!consumed) return; applyItemEffects(this.stats, consumed); this.sound.play('consume', 0.16); this.raiseNoise(BALANCE.sound.eatDrinkNoiseRadius); this.message = `${consumed.name} gegessen.`; }); }
  private startConsumeDrink() { const item = this.inventory.entries().find((entry) => entry.type === 'drink' && entry.canUse); if (!item) { this.message = 'Kein Getränk im Inventar.'; return; } this.startAction(`Trinken: ${item.name}`, 2.2, () => { const consumed = this.inventory.useBestDrink(); if (!consumed) return; applyItemEffects(this.stats, consumed); this.sound.play('consume', 0.16); this.raiseNoise(BALANCE.sound.eatDrinkNoiseRadius); this.message = `${consumed.name} getrunken.`; }); }
  private startUseMedical() { const item = this.inventory.entries().find((entry) => entry.type === 'medical' && entry.canUse); if (!item) { this.message = 'Keine Medizin im Inventar.'; return; } this.startAction(`Medizin: ${item.name}`, item.id.includes('bandage') ? 4.6 : 3.2, () => { const consumed = this.inventory.useMedical(); if (!consumed) return; applyItemEffects(this.stats, consumed); this.sound.play('consume', 0.14); this.message = `${consumed.name} verwendet.`; }); }
